...
class AutoBuyBot:
    ...
    def register_handlers(self):
        ...
        self.dp.message.register(self.menu_cmd, Command("menu"))
    ...
    async def run_polling(self):
        """Run bot in polling mode (для локального тестирования)"""
        await self.init()
        try:
    ...
    async def run_webhook(self):
        """Run bot in webhook mode (для хоста)"""
        await self.init()
        ...
        app = web.Application()
        app.router.add_post(f"/bot/{TELEGRAM_TOKEN}", handle_update)
        ...
        if WEBHOOK_URL:
            await self.bot.set_webhook_url(f"{WEBHOOK_URL}/bot/{TELEGRAM_TOKEN}")
            logger.info(f"Webhook set to {WEBHOOK_URL}/bot/{TELEGRAM_TOKEN}")
...
async def main():
    ...
    # Choose mode based on WEBHOOK_URL
    if WEBHOOK_URL:
